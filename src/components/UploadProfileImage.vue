    ...
    <q-card flat :style="$q.screen.lt.sm ? 'max-width:100%' : 'min-width:412px'" bordered>
        ...
        <q-card-section class="text-center q-py-lg" style="min-width:300px">
            ...
            <q-avatar size="16rem" v-else>
                <q-img spinner-color="white" :ratio="size.width / size.height" :src="cropImg" spinner-size="82px"
                    fit="contain" :style="$q.platform.is.mobile ? 'max-width: 300px' : 'max-width:500px'">
                ...
                <q-btn color="red-4" icon="sym_r_delete" round unelevated dense v-if="cropImg"
                    @click.stop="clearImage()" class="absolute q-card--bordered" style="bottom: 10px; right: 10px">
    ...
    <q-dialog v-model="dialogCrop" persistent :position="$q.screen.lt.sm ? 'right' : 'standard'">
        <q-card style="max-height: 90vh" class="scroll" flat bordered>
            ...
            <q-card-section class="avatar">
                <vue-cropper :src="imgSrc" ref="refCropper" guides :aspect-ratio="size.width / size.height"
                    @ready="$q.loading.hide()" style="max-height: 50vh" :class="$q.screen.lt.sm ? 'scroll' : ''" />
            </q-card-section>
            <q-card-section class="q-py-sm bg-grey-2">
                <div class="row">
                    <div class="col q-gutter-sm">
                        <q-btn color="white" round unelevated text-color="primary" class="q-card--bordered"
                            icon="sym_r_crop_free" @click="resetCropper" v-if="!$q.screen.lt.sm" />
                        <q-btn color="white" round unelevated text-color="primary" class="q-card--bordered"
                            icon="sym_r_rotate_left" @click="rotateLeft" />
                        <q-btn color="white" round unelevated text-color="primary" class="q-card--bordered"
