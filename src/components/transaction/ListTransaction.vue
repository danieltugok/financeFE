      ...
      <q-td :props="props">
        <div class="tw-flex tw-items-center">
          <q-icon
            ...
            :color="props.row.referenceCategoryBalance?.sub_category_balance?.icon ? 'primary' : 'grey'" size="32px"
            class="q-mr-md" />
          <div class="tw-flex tw-flex-col">
...
const { getTotalBalance, totalBalance, getTransactions, transactions, queryTransaction, setQueryTransaction, countTransactions, getBalanceService } = useTransactionComposable()
...
const calculateProcessedTableData = () => {
  ...
  if (transactions.value.length > 0) {
    ...
    let finalBalance: number = totalBalance.value ? totalBalance.value?.totalAmount : 0;
    let totalAmout = transactions.value.reduce((accumulator, currentValue) => {
      return accumulator + currentValue?.debit
    }, 0);
    let initialBalance = finalBalance - totalAmout;
    for (let row of transactions.value) {
      initialBalance += row.debit;
      processedData.push({ ...row, balance: initialBalance });
