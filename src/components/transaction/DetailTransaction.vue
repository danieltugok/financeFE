  ...
  <q-card flat style="width: 600px;" bordered>
    ...
    <q-card-section class="q-pt-none">
      <q-list>
        ...
        <q-separator v-if="transaction.id" />
        <q-item>
          <q-item-section side>
            ...
            <q-item-label lines="2">
              ...
              <div class="q-gutter-md" style="min-width: 300px">
                <input-date :transitionDate="transaction.date" @update="dateUpdated" dense outlined
                  clear-icon="sym_r_close" clearable :bg-color="$q.dark.isActive ? 'grey-10' : 'white'" />
              </div>
            </q-item-label>
          </q-item-section>
        </q-item>
...
const saveTransaction = async () => {
  if (props.id) {
    ...
    if (response?.status === 200 || response?.status === 201) {
      notify('positive', 'Transaction saved successfully', '');
      emit('updated');
...
watch(() => props.id, () => {
  ...
  else setTransaction({ user: { name: user.value?.name, email: user.value?.email } });
}, { immediate: true });
