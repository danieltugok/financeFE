<template>
  <q-table :rows="users" row-key="id" :key="Date.now()" flat :columns="columns" :pagination="pagination" hide-pagination
    class="table-card bg-transparent" @row-click="onDetaill">
    <template #body-cell-name="props">
      <q-td :props="props">
        <div class="tw-flex tw-flex-row items-center q-gutter-x-sm">
          <q-avatar v-if="props.row.imageProfile" font-size="25px" size="28px">
            <img :src="props.row.imageProfile" style="width: 28px; height: 28px;">
          </q-avatar>
          <q-avatar v-else size="28px" font-size="33px" color="primary" text-color="white" icon="mdi-account" />
          <div class="tw-flex tw-flex-col">
            <span>{{ props.row.name }}</span>
            <span class="tw-text-neutral-500">{{ props.row.email }}</span>
          </div>
        </div>
      </q-td>
    </template>
  ...
  <q-dialog v-model="confirmDelete" persistent>
    <q-card>
      ...
      <q-card-actions align="right">
        ...
        <q-btn color="negative" class="tw-px-3" :label="$t('delete')" dense unelevated @click="clickedToDeleteUser" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
